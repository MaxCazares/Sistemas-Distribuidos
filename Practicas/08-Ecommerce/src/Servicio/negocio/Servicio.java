...
public class Servicio {
            ...
            try {
                ...
                try {
                    ...
                    if (articulos_busqueda.size() > 0) {
                        ...
                        return Response.status(400).entity(j.toJson(new Error("No hay articulos que coincidan :(")))
                                .build();
                    }
    ...
    public Response comprar(@FormParam("articulo") Articulo articulo) throws Exception {
        ...
        if (articulo.cantidad < 0)
            return Response.status(400)
                    .entity(j.toJson(new Error("Se debe ingresar una cantidad de articulos a comprar valida"))).build();
        ...
        try {
            ...
            if (articulo.cantidad <= stock) {
                ...
                return Response.status(400)
                        .entity(j.toJson(new Error("No hay suficientes articulos para la compra :("))).build();
            }
    ...
    // eliminarArticulo no funciona correctamente
    @POST
    @Path("eliminarArticulo")
    @Consumes(MediaType.APPLICATION_FORM_URLENCODED)
    @Produces(MediaType.APPLICATION_JSON)
    public Response eliminar(@FormParam("articulo") Articulo articulo) throws Exception {
        Connection conexion = pool.getConnection();
        int stock = 0;
        int idArticulo = 0;

        if (articulo.cantidad < 0)
            return Response.status(400).entity(j.toJson(new Error("Ingrese una cantidad valida"))).build();

        try {
            PreparedStatement stmt_1 = conexion
                    .prepareStatement("UPDATE articulos SET cantidadAlmacen=? WHERE idArticulo=?");
            try {
                stmt_1.setInt(1, articulo.cantidad);
                stmt_1.setInt(2, articulo.idArticulo);
                stmt_1.executeUpdate();
            } finally {
                stmt_1.close();
            }
            ...
            PreparedStatement stmt_2 = conexion
                    .prepareStatement("DELETE carrito_compra WHERE idArticulo=?");
            try {
                stmt_2.setInt(1, articulo.idArticulo);
                stmt_2.executeUpdate();
            } finally {
                stmt_2.close();
            }
        ...
        return Response.ok().entity(j.toJson(idArticulo)).build();
    }
}