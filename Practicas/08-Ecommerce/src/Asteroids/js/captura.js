...
const nombreImagenproducto = document.querySelector('#nombreImagen');
const imagenProducto = document.querySelector("#imagenProducto");

let foto = null;

const readSingleFile = (files, imagen) => {
    let file = files[0];
    if (!file) return;
...
const limpiarDatos = () => {
    ...
    nombreImagenproducto.value = '';
    imagenProducto.src = '../img/usuarioSinFoto.jpg';
}

const capturarDatos = () => {
    let cliente = new WSClient("/Servicio/rest/ws");
    cliente.post("registrarArticulo", {
        articulo: {
